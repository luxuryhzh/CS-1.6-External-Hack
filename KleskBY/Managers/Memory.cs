...
namespace TopHack.Managers
{
    internal class MemoryManager
    {
        ...
        public static void Initialize(int ProcessID) => m_pProcessHandle = Globals.Imports.OpenProcess(PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE, false, ProcessID);
        ...
        {
            return Globals.Imports.CutString(Encoding.ASCII.GetString(ReadMemory(offset, size)));
        }
        public static string ReadStringUnicode(int offset, int size)
        {
            return Globals.Imports.CutString(Encoding.Unicode.GetString(ReadMemory(offset, size)));
        }
        public static string ReadStringUTF(int offset, int size)
        {
            return Globals.Imports.CutString(Encoding.UTF8.GetString(ReadMemory(offset, size)));
        }
